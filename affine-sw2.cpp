/*
 * @Author: your name
 * @Date: 2021-03-14 11:08:43
 * @LastEditTime: 2021-03-14 11:39:47
...
//   aUnit[0][0]->O = max3(aUnit[0][0]->X, aUnit[0][0]->Y, aUnit[0][0]->M);
    // aUnit[0][0]->O = max2(aUnit[0][0]->X, aUnit[0][0]->Y);
    aUnit[0][0]->O = 0;
  for (i = 1; i <= m; i++) { //第一列
    aUnit[i][0]->X = d + (i - 1) * e;
    ...
    aUnit[i][0]->O = max2(aUnit[i][0]->X, aUnit[i][0]->Y);
    aUnit[i][0]->W1 = 1;
  }
  for (j = 1; j <= n; j++) {
    ...
    aUnit[0][j]->O = max2(aUnit[0][j]->X, aUnit[0][j]->Y);
    aUnit[0][j]->W3 = 1;
  }
  // 将字符串都变成大写
  strUpper(s);
  ...
  for (i = 1; i <= m; i++) {
    ...
    //   aUnit[i][j]->X = max2(aUnit[i - 1][j]->X + e, aUnit[i - 1][j]->M + d);
      aUnit[i][j]->X = max2(aUnit[i - 1][j]->X + e, aUnit[i - 1][j]->O + d);
      // Y来自Y的extend，或者M的open，从左边过来
    //   aUnit[i][j]->Y = max2(aUnit[i][j - 1]->Y + e, aUnit[i][j - 1]->M + d);
      aUnit[i][j]->Y = max2(aUnit[i][j - 1]->Y + e, aUnit[i][j - 1]->O + d);
    ...
    aUnit[i][j]->O = max3(aUnit[i][j]->X, aUnit[i][j]->Y, aUnit[i-1][j-1]->O + f);
      if (aUnit[i][j]->O == aUnit[i][j]->X)
        aUnit[i][j]->W1 = 1;
    //   if (aUnit[i][j]->O == aUnit[i][j]->M)
    if (aUnit[i][j]->O == aUnit[i-1][j-1]->O + f)
        aUnit[i][j]->W2 = 1;
